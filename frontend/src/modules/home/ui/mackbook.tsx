...
export function Mackbook({
  ...
  const [isLaunch, isSetLaunch] = useState(true);
  const [isBrowser, isSetBrowser] = useState(false);

  const launch = () => {
    isSetLaunch(false);
    setTimeout(() => isSetBrowser(true), 2000);
  };
        ...
        <AnimatePresence initial={false}>
          {isLaunch && (
            ...
            >
              ...
              />
              <div
                className="absolute left-[40%] top-[52%] text-2xl  cursor-pointer items-center animate-bounce flex"
                onClick={launch}
              >
                <Image
                  src="/logo.svg"
          ...
          {isBrowser && (
            ...
            <Browser
              className="size-full"
              url="DigitalTrade.com"
              videoSrc="/digitaltrade.webm"
            />
            </motion.div>
          )}
        </AnimatePresence>
